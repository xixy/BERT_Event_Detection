...
flags.DEFINE_float("root_weight", None,
                   "Weight for root-level classification.")

flags.DEFINE_float("coarse_weight", None,
                   "Weight for coarse-level classification.")
...
class InputFeatures(object):
  ...
  def __init__(self,
    ...
    self.label_root_ids = label_root_ids
    self.label_coarse_ids = label_coarse_ids
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  ntokens = []
  segment_ids = []
  label_ids = [] # fine-level label
  label_root_ids = [] # root-level label
  label_coarse_ids = [] # coarse-level label

  ntokens.append("[CLS]")
  segment_ids.append(0)
  label_ids.append(label_map["[CLS]"])
  label_root_ids.append(label_util.root2id["[CLS]"])
  label_coarse_ids.append(label_util.coarse2id["[CLS]"])
  for i, token in enumerate(tokens):
    ntokens.append(token)
    segment_ids.append(i)
    label_ids.append(label_map[labels[i]])
    root_label = label_util.fine2root[labels[i]]
    label_root_ids.append(label_util.root2id[root_label])
    coarse_label = label_util.fine2coarse[labels[i]]
    label_coarse_ids.append(label_util.coarse2id[coarse_label])
  ...
  while len(input_ids) < max_seq_length:
    ...
    ntokens.append("[PAD]")
    label_root_ids.append(label_util.root2id["[PAD]"])
    label_coarse_ids.append(label_util.coarse2id["[PAD]"])
  assert len(input_ids) == max_seq_length
  assert len(mask) == max_seq_length
  assert len(segment_ids) == max_seq_length
  assert len(label_ids) == max_seq_length
  assert len(label_root_ids) == max_seq_length
  assert len(label_coarse_ids) == max_seq_length
  ...
  if ex_index < 3:
    ...
    tf.logging.info("label_root_ids: %s" % " ".join([str(x) for x in label_root_ids]))
    tf.logging.info("label_coarse_ids: %s" % " ".join([str(x) for x in label_coarse_ids]))
  ...
  feature = InputFeatures(
    ...
    label_ids=label_ids,
    label_root_ids=label_root_ids,
    label_coarse_ids=label_coarse_ids
  )

  return feature, ntokens, label_ids, label_root_ids, label_coarse_ids
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    feature,ntokens,label_ids,label_root_ids,label_coarse_ids = convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer)

    batch_tokens.extend(ntokens)
    batch_labels.extend(label_ids)
    batch_root_labels.extend(label_root_ids)
    batch_coarse_labels.extend(label_coarse_ids)
    ...
    features["label_root_ids"] = create_int_feature(feature.label_root_ids)
    features["label_coarse_ids"] = create_int_feature(feature.label_coarse_ids)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "label_root_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "label_coarse_ids": tf.FixedLenFeature([seq_length], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, label_root_ids, label_coarse_ids, num_labels, use_one_hot_embeddings):
  """
  ...
  with tf.variable_scope("loss"):
    ...
    # # shape = (batch size * seq length, num_labels)
    # logits = tf.reshape(logits, [-1, num_labels])

    # 计算root-label的任务
    with tf.variable_scope("root_loss"):
      # 1. 先做logits的映射，用sum的方式
      # shape  = (batch size * seq length, 4)
      logits_root = tf.matmul(tf.reshape(logits, [-1, num_labels]), np.array(label_util.fine2rootmapper, dtype=np.float32))
      # shape  = (batch size, seq length, 4)
      logits_root = tf.reshape(logits_root, [-1,length, len(label_util.root_labels)])
      # 计算loss
      # shape = (batch size, sequence length)
      loss_root = tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits = logits_root,
        labels = label_root_ids
        )
      # 进行mask
      loss_root = tf.boolean_mask(loss_root, input_mask)

      loss_root = tf.reduce_mean(loss_root)

    # 计算coarse-label的任务
    with tf.variable_scope("coarse_loss"):
      # 1. 先做logits的映射，用sum的方式
      # shape  = (batch size * seq length, 11)
      logits_coarse = tf.matmul(tf.reshape(logits, [-1, num_labels]), np.array(label_util.fine2coarsemapper, dtype=np.float32))
      # shape  = (batch size, seq length, 11)
      logits_coarse = tf.reshape(logits_coarse, [-1, length, len(label_util.coarse_labels)])
      # 计算loss
      loss_coarse = tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits = logits_coarse,
        labels = label_coarse_ids
        )
      # 进行mask
      loss_coarse = tf.boolean_mask(loss_coarse, input_mask)

      loss_coarse = tf.reduce_mean(loss_coarse)

    with tf.variable_scope("weighted_sum"):
      tf.logging.info("root_weight " + str(FLAGS.root_weight))
      tf.logging.info("coarse_weight " + str(FLAGS.coarse_weight))
      loss += FLAGS.root_weight * loss_root
      loss += FLAGS.coarse_weight * loss_coarse
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    label_root_ids = features["label_root_ids"]
    label_coarse_ids = features["label_coarse_ids"]
    ...
    (total_loss, logits, predicts) = create_model(
        ...
        label_root_ids, label_coarse_ids, num_labels, use_one_hot_embeddings)
...
if __name__ == "__main__":
  ...
  flags.mark_flag_as_required("root_weight")
  flags.mark_flag_as_required("coarse_weight")
